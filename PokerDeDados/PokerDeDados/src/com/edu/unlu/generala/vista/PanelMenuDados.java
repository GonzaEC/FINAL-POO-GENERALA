...
public class PanelMenuDados extends JPanel {
    ...
    public PanelMenuDados(ControladorGenerala controlador) {
        ...
        javax.swing.border.Border bordeNormal = BorderFactory.createEmptyBorder(4, 4, 4, 4);
        ...
        for (int i = 0; i < CANT_DADOS; i++) {
            ...
            botonesDados[i].setPreferredSize(new Dimension(DADO_SIZE + 10, DADO_SIZE + 10));
            botonesDados[i].setMinimumSize(new Dimension(DADO_SIZE, DADO_SIZE));
            botonesDados[i].setMaximumSize(new Dimension(DADO_SIZE + 10, DADO_SIZE + 10));

            botonesDados[i].setContentAreaFilled(false);
            botonesDados[i].setFocusPainted(false);

            botonesDados[i].setBorderPainted(true);
            botonesDados[i].setBorder(bordeNormal);

            botonesDados[i].addItemListener(e -> {
    ...
    public void actualizarEstadoYVista() {
        ...
        if (j == null) {
            lblJugador.setText("Esperando inicio...");
            lblJugador.setForeground(Color.WHITE);
            lblTiradasRestantes.setText("-");
            deshabilitarBotones();
            return;
        }
        ...
        if (miTurno) {
            lblJugador.setText("¡ES TU TURNO! (Juega: " + j.getNombre() + ")");
            lblJugador.setForeground(Color.GREEN); // Resalta en verde

            lblTiradasRestantes.setText("Tiradas restantes: " + controlador.getTiradasRestantes());
            actualizarBotones();
        } else {
            lblJugador.setText("Esperando a que juegue " + j.getNombre() + "...");
            lblJugador.setForeground(Color.WHITE);

            lblTiradasRestantes.setText("Por favor, espere su turno.");
            deshabilitarBotones();
        }
    }
